...
export const forecourtReducer = (
  state: ForecourtState,
  action: ForecourtAction
): ForecourtState => {
  switch (action.type) {
      ...
      return {
        ...
        fuellingPointElements: [
          ...action.payload.map((fuellingPointElement) => ({
            ...
            w: 1,
            h: 1,
            Transactions: fuellingPointElement.Transactions.filter(
              (transaction) => transaction.TransactionLockId === 0
            ),
          })),
        ],
    ...
    case "changeFuellingPointMainState":
      return {
        ...state,
        fuellingPointElements: state.fuellingPointElements.map(
          (fuellingPoint) => {
            if (
              fuellingPoint.FuellingPointId === action.payload.fuellingPointId
            ) {
              fuellingPoint.FuellingPointMainState =
                action.payload.fuellingPointMainState;
            }
        ...
        ticket: {
          ...
          Transactions: state.ticket.Transactions.filter((transaction) => {
            return !(
              ...
              action.payload.transaction.TransactionSequenceNumber &&
              transaction.FuellingPointId ===
              action.payload.transaction.FuellingPointId
            );
          }),
        },
      };
    case "changeTransaction":
      return {
          ...
          (fuellingPoint) => {
            if (
              ...
              if (transactionToUpdate) {
                ...
                transactionToUpdate.TransactionLockId =
                  action.payload.transaction.TransactionLockId;
              } else {
                // La transacción no existe, agrégala al array si no es prepago
                if (!isPrepay(action.payload.transaction)) {
                  fuellingPoint.Transactions = [
                    ...fuellingPoint.Transactions,
              ...
              return {
                ...fuellingPoint,
                Transactions: fuellingPoint.Transactions.filter(
                  (t) => t.TransactionLockId === 0
                ),
              };
            } else {
              return fuellingPoint;
        ...
        ticket: {
          ...
          Transactions:
            action.payload.transaction.TransactionLockId !==
              parseInt(new URLSearchParams(window.location.search).get("posid")!)
              ? state.ticket.Transactions.filter(
                (transacion) =>
                  transacion.TransactionSequenceNumber !==
                  action.payload.transaction.TransactionSequenceNumber ||
                  (isPrepay(transacion) &&
                    transacion.FuellingPointId !==
                    action.payload.transaction.FuellingPointId)
              )
